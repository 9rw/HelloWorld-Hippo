...
export default function Nav() {
  ...
  return (
    <nav className="z-[999] fixed top-0 flex justify-between items-center px-10 py-4 w-full h-[164px] bg-white text-primary-background border-b-2 border-primary-foreground/20 shadow-lg">
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="flex flex-row gap-6 justify-start items-center max-sm:hidden">
          {["CB2", "LX Building", "SIT Building"].map((building, index) => (
            <Link key={index} href={`#${building.slice(0, 2)}`}>
              <Button
                variant={"building"}
      ...
      <div className="relative w-max h-[100px] mt-[10px] flex items-start flex-row max-lg:mt-[5px] max-md:mt-0">
        ...
        <Popover>
          ...
          <PopoverContent className="flex flex-col gap-2 p-2">
            <Button variant={"building"} className="relative before:rounded-md">
              Booking History
            </Button>
            <Button variant={"building"} className="relative before:rounded-md">
              Report Rooms
            ...
            <Button variant={"link"} className="w-max p-0 self-center">
              contact
            </Button>
          </PopoverContent>
        </Popover>
